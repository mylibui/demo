...
def build_objective_uae(X_train, y_train, skf):
    return lambda trial: objective_uae(trial, X_train, y_train, skf)


def build_objective_uvae(X_train, y_train, skf):
    return lambda trial: objective_uvae(trial, X_train, y_train, skf)


def build_objective_sae(X_train, y_train, skf):
...
def objective_uae(trial, X_train, y_train, skf):
    ...
    for train_idx, val_idx in skf.split(X_train, y_train.flatten()):
        ...
        history = model.fit(
            ...
            epochs=5,
            batch_size=128,
            validation_data=(X_val_fold[y_val_fold.flatten() == 0], None),
            verbose=0,
        )
...
def objective_uvae(trial, X_train, y_train, skf):
    ...
    for train_idx, val_idx in skf.split(X_train, y_train.flatten()):
        ...
        recon_error = calculate_reconstruction_error(model, X_val_fold)
        if np.any(np.isnan(recon_error)):
            recon_error = np.nan_to_num(
                recon_error, nan=np.finfo(recon_error.dtype).max
            )
        roc_auc = roc_auc_score(y_val_fold.flatten(), recon_error)
...
def objective_sae(trial, X_train, y_train, skf):
    ...
    l2_lambda = trial.suggest_float(
        "l2_lambda", 0.001, 0.1, log=True
    )  # L2-Regularisierungsfaktor optimieren
    # Initialisiere ROC-AUC über die Folds
    roc_auc_scores = []
    ...
    for train_idx, val_idx in skf.split(X_train, y_train.flatten()):
        ...
        preds = (
            model.classify(
                model.encode(tf.convert_to_tensor(X_val_fold, dtype=tf.float32))
            )
            > 0.5
        ).numpy()
