...
def plot_loss_curves(
    ...
    if not isinstance(history, (tf.keras.callbacks.History, dict)):
        raise TypeError(
            "history muss ein tf.keras.callbacks.History-Objekt oder Dictionary sein"
        )
    if (
        not isinstance(figsize, tuple)
        or len(figsize) != 2
        or not all(isinstance(d, int) and d > 0 for d in figsize)
    ):
        raise TypeError("figsize muss ein Tuple von zwei positiven Ganzzahlen sein")
    valid_model_types = {"uae", "uvae", "sae", "svae"}
    if model_type not in valid_model_types:
        raise ValueError(
            f"model_type muss einer von {valid_model_types} sein, nicht {model_type}"
        )
    ...
    if "loss" in metrics:
        ...
        ax.annotate(
            f"End: {end_val:.4f}",
            xy=(len(history.history["loss"]) - 1, end_val),
            xytext=(5, -15),
            textcoords="offset points",
            color=colors["loss"],
            fontsize=10,
        ...
        if val_metrics_exist and "val_loss" in metrics:
            ...
            ax.annotate(
                f"End: {val_end:.4f}",
                xy=(len(history.history["val_loss"]) - 1, val_end),
                xytext=(5, -45),
                textcoords="offset points",
                color=colors["loss"],
                fontsize=10,
    ...
    if "reconstruction_loss" in metrics and ax_idx < n_plots:
        ...
        ax.plot(
            ...
            color=colors["reconstruction_loss"],
            linestyle=line_styles["training"],
            linewidth=2,
            label="Training",
        )
        ...
        ax.annotate(
            f"End: {end_val:.4f}",
            xy=(len(history.history["reconstruction_loss"]) - 1, end_val),
            xytext=(5, 10),
            textcoords="offset points",
            color=colors["reconstruction_loss"],
            fontsize=10,
        ...
        if val_metrics_exist and "val_reconstruction_loss" in metrics:
            ...
            ax.annotate(
                f"End: {val_end:.4f}",
                xy=(len(history.history["reconstruction_loss"]) - 1, val_end),
                xytext=(5, -15),
                textcoords="offset points",
                color=colors["reconstruction_loss"],
                fontsize=10,
    ...
    if "kl_loss" in metrics and ax_idx < n_plots and model_type in ["uvae", "svae"]:
        ...
        ax.annotate(
            f"End: {end_val:.4f}",
            xy=(len(history.history["kl_loss"]) - 1, end_val),
            xytext=(5, 10),
            textcoords="offset points",
            color=colors["kl_loss"],
            fontsize=10,
        ...
        if val_metrics_exist and "val_kl_loss" in metrics:
            ...
            ax.annotate(
                f"End: {val_end:.4f}",
                xy=(len(history.history["kl_loss"]) - 1, val_end),
                xytext=(5, -15),
                textcoords="offset points",
                color=colors["kl_loss"],
                fontsize=10,
    ...
    if (
        "classification_loss" in metrics
        and ax_idx < n_plots
        and model_type in ["sae", "svae"]
    ):
        ax = axes[ax_idx]
        ax.plot(
            history.history["classification_loss"],
            color=colors["classification_loss"],
            linestyle=line_styles["training"],
            linewidth=2,
            label="Training",
        )
        ...
        ax.annotate(
            f"End: {end_val:.4f}",
            xy=(len(history.history["classification_loss"]) - 1, end_val),
            xytext=(5, 10),
            textcoords="offset points",
            color=colors["classification_loss"],
            fontsize=10,
        ...
        if val_metrics_exist and "val_classification_loss" in metrics:
            ...
            ax.annotate(
                f"End: {val_end:.4f}",
                xy=(len(history.history["classification_loss"]) - 1, val_end),
                xytext=(5, -15),
                textcoords="offset points",
                color=colors["classification_loss"],
                fontsize=10,
    ...
    if save_path is None:
        ...
        save_path = os.path.join(
            folder_path, f"loss_curves_{model_type}_{timestamp}.png"
        )
